...
9. Не используй внешние JS-библиотеки — только чистый HTML/CSS/JS.`;
  ...
  async function handleGenerate() {
    ...
    setPhase("generating");
    setStatusMsg("Придумываю структуру...");
    setGeneratedHTML("");
    ...
    try {
      ...
      setChatHistory([
        { role: "user", content: `Создай сайт: ${prompt}` },
      ...
      setPhase("error");
      setStatusMsg("Ошибка при генерации. Проверь API ключ в .env файле.");
    }
  }
  ...
  async function handleEdit() {
    ...
    setPhase("generating");
    setStatusMsg("Вношу правки...");

    const msgs = [
      ...chatHistory,
      { role: "user", content: `Измени сайт: ${editPrompt}. Верни полный обновлённый HTML.` },
    ];
    try {
      const html = await generate(msgs);
      const cleaned = html.replace(/^```html?\n?/, "").replace(/\n?```$/, "").trim();
      ...
      setPhase("error");
      setStatusMsg("Ошибка при редактировании.");
    }
  }
    ...
    <div style={{
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        ::-webkit-scrollbar-track { background: #1E2440; }
        ...
        .prompt-textarea::placeholder { color: rgba(248,247,255,0.35); }
        .btn-primary {
        ...
        .chip:hover { background: rgba(124,58,237,0.3); color: #F8F7FF; border-color: rgba(124,58,237,0.5); }
        .tab {
          padding: 8px 16px;
          border-radius: 8px;
          font-size: 13px;
        ...
        .tab-inactive { background: transparent; color: rgba(248,247,255,0.4); }
        .tab-inactive:hover { color: rgba(248,247,255,0.7); }
        @keyframes pulse {
        ...
        .edit-input::placeholder { color: rgba(248,247,255,0.3); }
        .glass-card {
          ...
          <div style={{ paddingTop: 60, paddingBottom: 80 }}>
            <div style={{ textAlign: "center", marginBottom: 52 }}>
              <div style={{
                ...
                <span style={{ fontSize: 10 }}>●</span> Работает на Claude AI
              </div>
              <h1 style={{
              ...
              <p style={{ fontSize: 17, color: "rgba(248,247,255,0.55)", maxWidth: 480, margin: "0 auto", lineHeight: 1.6 }}>
                Искусственный интеллект создаст полноценный сайт по твоему описанию. Без кода, за секунды.
              </p>
            </div>
            ...
            <div className="glass-card" style={{ padding: 28, maxWidth: 700, margin: "0 auto 32px" }}>
              <textarea
                ...
                rows={4}
                placeholder="Например: лендинг для барбершопа в стиле минимализм с прайсом, фото мастеров и формой записи..."
                value={prompt}
            ...
            <div style={{ maxWidth: 700, margin: "0 auto" }}>
              <p style={{ fontSize: 12, color: "rgba(248,247,255,0.3)", marginBottom: 12, textAlign: "center", letterSpacing: "0.5px", textTransform: "uppercase" }}>
                Попробуй пример
              </p>
        ...
        {isGenerating && (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "70vh", gap: 28 }}>
            ...
            <div style={{ textAlign: "center" }}>
              ...
              <p style={{ color: "#A78BFA", fontSize: 16, minHeight: 24 }}>
                <TypingText text={statusMsg} key={statusMsg} />
              </p>
            </div>
          ...
          <div style={{ paddingBottom: 40 }}>
            ...
            <div className="glass-card" style={{ height: "60vh", overflow: "hidden", marginBottom: 16 }}>
              {activeTab === "preview" ? (
                ...
                <div className="code-block">{generatedHTML}</div>
              )}
            </div>
            ...
            <div className="glass-card" style={{ padding: "14px 16px" }}>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <input
                  ...
                  placeholder='Например: "Сделай фон тёмным и добавь кнопку WhatsApp"'
                  value={editPrompt}
                  onChange={e => setEditPrompt(e.target.value)}
                  onKeyDown={e => { if (e.key === "Enter") handleEdit(); }}
                />
                <button className="btn-primary" onClick={handleEdit} disabled={!editPrompt.trim()} style={{ padding: "11px 20px", whiteSpace: "nowrap" }}>
                  Изменить
                </button>
              </div>
            </div>
